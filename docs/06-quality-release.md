...
Every `passed` protected report—physical E2E, performance, iOS signed-archive inspection, and Android signed-AAB inspection—must carry a fresh Ed25519 signature from an out-of-band trusted protected runner. The canonical signed payload includes the full report plus its `report_id`, exact source commit, and artifact digest; the release environment supplies the trusted public-key map. A JSON boolean such as `attested: true`, an arbitrary runner label, an expired signature, or a signature from an untrusted key is rejected. The iOS archive and Android AAB report signer must also be the runner named by the evidence record.
...
The Builder-to-keyboard vertical slice has a separate source/integration gate: `node scripts/check-shortcut-vertical-slice.mjs`. It checks, independently on iOS and Android, immutable Skill id/version/digest creation, exact deploy confirmation, an explicit `Add To My Keyboard` action, persistent local installation, A-Z assignment publication, closed local IME execution, and last-known-good/local snapshot reload seams. The existing wildcard runner picks up [`scripts/test/shortcut-vertical-slice.test.mjs`](../scripts/test/shortcut-vertical-slice.test.mjs), so a missing seam fails the test instead of being hidden by a fixture. `--strict` is the release fail-closed form. A passing source report is not a device result: keyboard registration, process restart, third-party app behavior, and physical execution remain `not_proven` until protected iOS/Android evidence is attached. The current source report and invariant inventory are [`docs/shortcut-vertical-slice-contract.json`](shortcut-vertical-slice-contract.json).

The CI lanes execute the private-Skill-to-A-Z assignment flow on an iOS Simulator and Android Emulator in addition to core tests, lint, and native builds. They are intentionally classified as local UI/source/build evidence; neither lane is a physical-device test, VoiceOver/TalkBack run, system-keyboard registration proof, or third-party-app E2E. A protected E2E/performance/archive record may be marked `passed` only when its candidate source SHA and artifact digest are full, exact matches at the top level and in every protected run/measurement, with `environment: "protected_device"`, an independently verified protected-runner attestation bound to the run/device/candidate, and a stable concrete device identity (class, model, and OS version). The gate requires a strict 64-byte Ed25519 signature over the complete canonical run (including every result and evidence digest) against a verifier public key supplied out-of-band through `MOBILE_AI_KEYBOARD_TRUSTED_ATTESTATION_KEYS_JSON`; RSA/ECDSA keys, malformed base64, candidate-declared keys, expired/future/overlong windows, and reused nonces or verification IDs fail closed. Every run must itself be `status: "passed"` and carry one passed, evidence-bound result for each required scenario, field class, lifecycle event, and app; coverage cannot be assembled from free-form label unions or legacy `attested` fields. E2E proof must cover every required scenario, including screen-reader and font-scale behavior, and must name the platform's physical accessibility tool (`voiceover` or `talkback`). Simulator, emulator, UI-test-only, fixture, JVM, and self-attested markers are rejected from passed records.
