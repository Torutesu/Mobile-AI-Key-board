...
- Versioned keyboard customization and Japanese workflow-pack fixture models plus qualification-budget surfaces. Session/revocation boundaries preserve ordinary preferences while account deletion resets them. The Android IME now applies persisted theme, compact/standard/large key height, left/right one-handed width, character preview, sound, and haptic settings at render time; workflow-pack execution, complete font-scale/contrast geometry, iOS extension parity, and physical process-death synchronization remain `not_proven`.
