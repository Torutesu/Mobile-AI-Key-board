...
- Skill Keys management with QWERTY preview, search, A-Z add/reassign/remove, occupied-key selection, explicit Swap/Replace, save-before-test local fixture preview, success confirmation, version/digest-bound content-free App Group snapshots, last-known-good fallback, bound-key accessibility actions, tap-to-type preservation, 450 ms long-press activation, a visible non-hold Skill palette, and routing into the existing Capture Review/Result Preview flow. Skill Keys sharing requests Full Access while ordinary typing remains available without it.
- Production Skill Key publication now fails closed unless the Host can resolve the shared App Group container; onboarding treats Full Access without App Group availability as incomplete. Cancel, editor transition, secure lock, dismissal, snapshot invalidation, and timeout erase command/result editor buffers as well as capture state.
- P0 input-safety hardening keeps bundled local transforms selection-only on both platforms, blocks empty selection instead of treating a Skill label as editor content, exposes Skill execution through both an accessibility action and a visible non-hold palette while ordinary accessibility click still types, transfers screen-reader focus on iOS review transitions, announces content-free Android mode changes, suppresses configured-off Android haptics/sounds, uses locale-stable letter mapping, invalidates Android review authority on IME view recreation, and reports delete/return mutation failures. Physical VoiceOver/TalkBack, Switch Control, font-scale geometry, and third-party editor qualification remain `not_proven`.
...
- Versioned keyboard customization and Japanese workflow-pack fixture models plus qualification-budget surfaces. Session/revocation boundaries preserve ordinary preferences while account deletion resets them. Android applies its persisted keyboard settings at render time. iOS now publishes content-free theme, key-size, left/right one-handed, and haptic settings through the App Group and reloads them when the extension appears; Swift round-trip and Simulator build evidence pass. Physical process-death synchronization, complete font-scale/contrast geometry, and device-level interaction quality remain `not_proven`.
...
- Each visible input-view attachment refreshes `EditorInfo` and `InputConnection` as a new capability boundary. `TYPE_NULL` fields fail closed, Copy/Undo revalidate the exact Skill activation, and TTL expiry automatically destroys result and Undo authority.
...
- Builder-to-IME vertical-slice source gate: both platforms expose immutable Skill identity, exact deploy confirmation, explicit `Add To My Keyboard` separation, A-Z assignment publication, closed local execution, and snapshot reload seams. This is source/integration evidence only; device qualification remains `not_proven`.
...
- Release-readiness adversarial tests require out-of-band trusted Ed25519 signatures for passed E2E, performance, iOS archive, and Android AAB evidence, including candidate/report binding, freshness, nonce/verification identity, forged-signature, and untrusted-key rejection.
...
- TypeScript, Swift, and Kotlin snapshot encoders intentionally validate their own native projections today. The checked-in fixture records `native_consumption_status: "native_unit_consumers"`, and the vertical-slice source gate confirms both native unit consumers are wired to the authoritative fixture. This proves only native unit-consumer coverage; canonical byte parity, cross-process runtime interoperability, physical-device behavior, and a server-synchronized registry remain `not_proven`.
- Search within the now-visible overflow palette, authenticated cross-device shortcut synchronization, and production Skill catalog installation. Physical A-Z assignment, conflict management, tap/hold/non-hold handling, and local Review routing are implemented foundations; Acti-class parity on representative third-party apps remains `not_proven` until device qualification.
